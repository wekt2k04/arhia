...
## En une phrase
**Milestones 0-9 et 11 terminés, application complète fonctionnelle de bout en bout, y compris en Docker Compose.** Migration en cours du vocabulaire métier français → anglais dans le code (plan à 4 patches, voir `~/.claude/plans/jazzy-booping-lovelace.md`) : **Patches 1/4, 2/4 ET 3/4 terminés, committés et poussés** — 211/211 tests verts, build + `npm run build` verts après chaque patch. **Seul le Patch 4/4 (Finalisation) reste** — il contient une opération destructive locale (`dotnet ef database drop`) explicitement signalée dans le plan, à ne pas lancer sans confirmation fraîche. Le reste de l'état applicatif (Docker Compose, NotebookLM, profils Ollama) est inchangé depuis le checkpoint précédent.

## Depuis le dernier checkpoint (2026-08-18, poste de travail — 3 patches dans la même session)

**Patch 1/4 puis Patch 2/4 exécutés** (voir entrées LOG.md du 2026-08-18 pour le détail complet), puis **Patch 3/4 exécuté à la suite sur demande explicite du porteur du projet** ("Continue").

**Patch 3/4 — Conversation & RAG** : renommage complet de l'orchestration conversationnelle (`RepondreConversationUseCase`→`AnswerConversationUseCase`, `EvenementConversation`/`FragmentTexte`/`ReponseTerminee`→`ConversationEvent`/`TextFragment`/`ResponseCompleted`, `IntentionConversation`→`ConversationIntent` avec valeurs `DocumentaryQuestion`/`CaseStatus`/`OutOfScope`, `ReponseConversation`→`ConversationResponse`) et du pipeline RAG (`ChunkDocumentaire`→`DocumentChunk` avec `CheminTitres`/`Contenu`→`TitlePath`/`Content`, tous les ports `IEmbeddingPort`/`IRerankerPort`/`IVectorSearchPort`/`ILlmRouterPort`/`ILlmGeneratorPort`/`IDocumentChunkerPort`, les adapters `Ollama*`/`Onnx*`/`Qdrant*`/`XlmRobertaTokenizer`/`MarkdownChunker`, `IngererCorpusUseCase`→`IngestCorpusUseCase`). `ChatController` réécrit (route `demander`→`ask`, événement SSE `termine`→`done`, champs JSON `texte`/`sourcee`→`text`/`sourced`) ; `AdminController` terminé (`CorpusOptions`, `ReindexCorpusResponse`). Frontend : route BFF `app/api/chat/demander`→`app/api/chat/ask`, champ `targetEmployeeId`, événement SSE `done`, champs `text`/`sourced` (`chat-widget.tsx`, `chat-message.tsx`).

**Décision technique prise sans interrompre le porteur du projet** (contrairement au point WorkflowStatus du Patch 2) : le prompt de classification du routeur conversationnel (`OllamaRouterAdapter`, libellés `DOCUMENTAIRE`/`STATUT_DOSSIER`/`HORS_PERIMETRE`) est resté **volontairement en français** — c'est un contrat de prompt calibré empiriquement avec le modèle (routage déjà imparfait, ~27% d'erreur connu), pas du vocabulaire de code à traduire mécaniquement ; le traduire aurait risqué de dégrader silencieusement un routage déjà fragile sans aucun bénéfice. Documenté par un commentaire dans le code lui-même.

**Nettoyage du fix temporaire posé au Patch 2** : le mapping `StatutEnFrancais` (ajouté en attendant que ce fichier soit repris) a été retiré et remplacé par sa version définitive `FormatStatusInFrench`, intégrée normalement puisque `AnswerConversationUseCase` est désormais réécrit en entier.

**Bug corrigé au passage** (régression silencieuse du Patch 2, sans impact visuel) : `notification-bar.tsx` lisait encore `dateReference` alors que le Patch 2 avait renommé `Notification.DateReference`→`ReferenceDate` côté backend — le champ n'étant jamais affiché dans l'UI, aucune régression visible, mais corrigé pour cohérence (`referenceDate`).

**⚠️ Point d'attention opérationnel (pas encore vérifié en conditions réelles)** : les clés de payload Qdrant ont changé (`cheminTitres`/`contenu` → `titlePath`/`content`). Si une collection Qdrant existante contient des documents indexés **avant** ce patch, les anciennes clés ne correspondent plus à ce que le code lit désormais — une question "documentaire" dans le chat échouera ou plantera tant que `POST api/admin/reindex-corpus` n'a pas été relancé. Docker n'était pas démarré sur ce poste au moment du patch, donc **pas vérifié en conditions réelles** — à faire avant tout test manuel du chat RAG. C'est le comportement explicitement prévu par le plan ("Impact DB: Qdrant (ré-indexation du corpus)"), pas un oubli.

Vérifications : `dotnet build` 0 erreur/0 warning, `dotnet test` 211/211 verts (même total qu'avant — renommage mécanique), `npx tsc --noEmit` et `npm run build` (frontend) verts.
...
1. **Demander confirmation explicite au porteur du projet avant de lancer le Patch 4** ("Finalisation") — plus encore que pour les patches précédents, car celui-ci contient une **opération destructive locale** : régénération d'une migration EF `InitialCreate` unique + `dotnet ef database drop`/`update` sur `agirh-sql` (efface le schéma/données de dev locales — acceptable, aucune donnée de prod derrière, mais à ne jamais lancer sans un "go" frais, pas seulement un "continue" générique). Contenu du reste du patch : `Program.cs` (renommages forcés restants + ses propres identifiants encore français, ex. `TrouverRacineDepot`), DTOs `AdminController` restants si besoin, grep final anti-oubli sur tout `src/`/`tests/`/`frontend/`, mise à jour des citations de code dans `docs/**/*.md`, HANDOFF final de la migration.
2. **Avant de lancer le Patch 4, relancer `POST api/admin/reindex-corpus`** (voir point d'attention Qdrant ci-dessus) et vérifier qu'une question RAG dans le chat fonctionne toujours — ce test n'a pas pu être fait cette session (Docker non démarré).
3. Sujets pré-existants toujours ouverts (indépendants de la migration, voir "Ce qui reste ouvert" plus bas) : vérification visuelle pixel du chat, routeur conversationnel (~27% de mauvais routage), jeu de Q/R gold à reconfirmer.
...
**Confirmer avec le porteur du projet le passage au Patch 4** de la migration vocabulaire, en particulier le drop/recreate de la base locale (voir ci-dessus). Au-delà de la migration, la suite reste **pas encore décidée** :
