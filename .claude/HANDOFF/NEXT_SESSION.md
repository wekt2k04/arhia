...
1. **Un vrai logo AGIRH existe maintenant** (`C:\Users\Wilfried\OneDrive\Bureau\presentation\assets\agirh_white_rgba.png` + variante non-transparente, hors dépôt) — utilisé dans la présentation, jamais branché dans le frontend. Le placeholder générique (icône `UsersRound`) est toujours dans `frontend/components/agirh-mark.tsx` : remplacer par une vraie `<Image>` Next.js reste une action concrète, pas juste une attente d'un fichier qui n'existait pas avant.
