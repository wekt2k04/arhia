...
## En une phrase
**Application inchangée sur le fond** depuis la fin de la migration vocabulaire (2026-08-18, milestones 0-9/11 fonctionnels de bout en bout). Cette session a porté sur les **livrables de communication** : rapport d'avancement PDF refait (3 pages, nouveaux diagrammes), correction du dossier de sauvegarde des captures d'écran Windows, et mise à jour du script de génération de la présentation PowerPoint de soutenance (citations de code redevenues exactes). **Une question factuelle reste bloquante avant de considérer le pptx et le rapport comme définitivement cohérents entre eux** — voir "Décisions en attente".
...
- **Nouveau — identité de l'encadrant, à trancher avant la soutenance.** `docs/presentations/generate_pptx.py` (slide 1) et `script_orateur.md` (ligne ~23, phrase d'ouverture prononcée) disent **« M. Issam MITAR »** (encadrant entreprise) — inchangé depuis le 17 août. Le rapport PDF fraîchement refait (`docs/rapport_avancement/rapport.tex` ligne 53) dit **« M. Moulay Rachid Didi Alaoui (superviseur), M. Saad (encadrant direct) »**. Ces deux affirmations n'ont pas pu être réconciliées automatiquement (aucune n'est dérivable du code) — à trancher : soit le rapport a raison et Issam MITAR doit être remplacé partout, soit le pptx/script ont raison et le rapport doit être corrigé, soit ce sont des rôles réellement différents (encadrant entreprise vs superviseur académique ENSA Safi) à afficher côte à côte. Question posée au porteur du projet en fin de session du 2026-08-19 (pas encore répondue à ce checkpoint).
