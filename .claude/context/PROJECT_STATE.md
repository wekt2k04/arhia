...
Le projet a été **entièrement remis à zéro** le 2026-08-14 (V7 → V8, voir `HISTORIQUE.md` à la racine). Le code V7 est supprimé ; les agents custom (`.claude/agents/`) ont été réalignés sur les décisions V8, mais **toute règle qui cite un fichier/chemin V7** doit être traitée comme obsolète tant que ce fichier n'existe pas réellement — vérifier avec `Glob`/`Grep` avant de s'appuyer dessus.

## Specs canoniques (racine du repo)
1. **`LOGIQUE_METIER.md`** — rôles, workflow onboarding/offboarding, RBAC, garde-fous IA. **Fait.**
2. **`STACK_TECHNIQUE.md`** — stack backend/frontend/données/IA. **Fait.**
3. **`ARCHITECTURE.md`** — hexagonal, arborescence de dossiers, diagrammes. **Fait.**
4. **`CHECKLIST.md`** — suivi milestone par milestone (statuts en émojis, livré, reste à faire). Mis à jour à la fin de chaque étape.
...
## Code existant (milestones 3-4, CHECKLIST.md)
...
## Décisions structurantes déjà actées (résumé — détail dans LOGIQUE_METIER.md)
...
- **.NET 8** en pratique (seul SDK installé — voir STACK_TECHNIQUE.md §1, .NET 10 visé à l'origine mais pas encore réévalué)
