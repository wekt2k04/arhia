...
*Dernière mise à jour : 2026-08-17, poste de travail (Windows). Ce fichier est **réécrit** à chaque checkpoint (pas un journal) — pour l'historique complet, voir `HANDOFF/LOG.md`.*

## En une phrase
**Milestones 0-9 et 11 terminés, application complète fonctionnelle de bout en bout, y compris en Docker Compose. Profil Ollama entreprise complet (`Maison`/`Entreprise` via `--launch-profile`). Racine du dépôt réorganisée** (documents de cadrage sous `docs/`, 2 checklists SMSI sources retirées — déjà absorbées ailleurs). Page de garde → connexion/inscription (cookie httpOnly) → chat en streaming réel (SSE token par token depuis Ollama, à travers un proxy BFF) → barre de notifications en direct — tout vérifié en HTTP réel, y compris `docker compose up` complet sur base fraîche. Il reste : la vérification visuelle **pixel** du chat retravaillé (seul le porteur du projet peut clore ce point en ouvrant son navigateur), le routeur conversationnel (~27% de mauvais routage, mis de côté volontairement), le jeu de Q/R gold à reconfirmer d'un seul tenant, et des chantiers annexes (endpoints de lecture/liste, cas particuliers §8).

## Depuis le dernier checkpoint (2026-08-17, poste de travail)
**Racine du dépôt réorganisée**, sur demande explicite du porteur du projet ("trop de .md à la racine") :
- `ARCHITECTURE.md`, `CHECKLIST.md`, `LOGIQUE_METIER.md`, `STACK_TECHNIQUE.md`, `HISTORIQUE.md`, `SUJET_STAGE.md` déplacés vers `docs/` (`git mv`, historique préservé). `CLAUDE.md` et `README.md` restent à la racine (imposés respectivement par l'outillage Claude Code et la convention GitHub). **Tous les documents canoniques sont donc maintenant sous `docs/CHECKLIST.md`/`docs/LOGIQUE_METIER.md`/etc.** — mettre à jour ses habitudes de chemin.
- Tous les renvois croisés corrigés dans ~35 fichiers (CLAUDE.md, README.md, les 5 agents `.claude/agents/`, `.claude/context/PROJECT_STATE.md`, `.claude/commands/`, commentaires `///` dans une quinzaine de fichiers `.cs`, `eval/gold_qa.json`, plusieurs routes `frontend/`, `scripts/download-models.ps1`, `HANDOFF/NEXT_SESSION.md`) — vérifié par grep exhaustif sur tout le dépôt, plus aucune mention bare de ces 6 noms de fichier hors `docs/`. **Exception volontaire** : les entrées historiques de `HANDOFF/LOG.md` n'ont pas été touchées (append-only, ne jamais réécrire le passé — cohérent avec `CLAUDE.md`).
- Les 2 fichiers checklist SMSI sources (`SMSI.ENR.10-1...`, `SMSI.ENR.10-2...`) — trouvés manquants du disque la veille sans suppression commitée — **formellement supprimés** (`git rm`) après confirmation avec le porteur du projet : leur contenu (rôles Onboarding/Offboarding, format ISO 27001/SMSI) est déjà entièrement absorbé et anonymisé dans `docs/LOGIQUE_METIER.md` (citation explicite : "Dérivé du document réel SMSI.ENR.10-1/2"), `docs/STACK_TECHNIQUE.md` et `corpus/`. Zéro référence dans le code (`.cs`/`.json`/scripts) vérifié par grep avant suppression.
- Build .NET revérifié vert (0 warning) après les édits de commentaires. `git status` propre, poussé sur `origin/master`.

**Profils de lancement `Maison`/`Entreprise` pour Ollama** (`src/Agirh.Api/Properties/launchSettings.json`), mis en place juste avant la restructuration, sur demande explicite du porteur du projet — détail complet dans `HANDOFF/LOG.md` :
- **`Maison`** : `Ollama__BaseUrl=http://localhost:11434`, Router+Generator = `phi4-mini:3.8b` (comportement inchangé).
- **`Entreprise`** : `Ollama__BaseUrl` = IP réelle du serveur entreprise (fournie et confirmée par le porteur du projet, **accessible uniquement depuis le réseau entreprise**), `Ollama__RouterModele=phi4-mini:3.8b` (identique à Maison, routage déjà calibré), `Ollama__GeneratorModele=qwen3.5:9b` (choisi avec le porteur du projet à partir du `tags.json` réel du serveur : 9.7B, contexte 262k).
- Lancement : `dotnet run --launch-profile Maison` (ou `Entreprise`), ou menu déroulant de l'IDE.
- **IP réelle jamais committée** (décision explicite du porteur du projet) : `launchSettings.json` réel dans `.gitignore` (comme `appsettings.Development.json`), `launchSettings.json.example` commité avec un placeholder. **Conséquence pratique pour tout autre appareil/session** : au prochain `git pull` ailleurs, ce fichier disparaît du disque — copier `launchSettings.json.example` vers `launchSettings.json` et renseigner l'IP réelle localement avant de relancer l'Api nativement.
...
3. En reprenant sur un autre appareil : `docs/` a bougé et `launchSettings.json` a disparu du disque (voir ci-dessus) — lire ce fichier avant de supposer quoi que ce soit sur les chemins.
...
## Ce qui reste ouvert
0. **Vérification visuelle *pixel* de `/chat` retravaillé** (avatars, streaming, sources en badges, auto-scroll) — la partie fonctionnelle est confirmée (200, vrais composants chargés, vraie session, voir `HANDOFF/LOG.md`), mais aucun outil de capture d'écran/navigateur automatisé n'était disponible dans les sessions récentes pour vérifier le rendu pixel. Les deux serveurs tournent déjà (`http://localhost:3000/chat`, compte `chattest@agirh.test`) — un simple coup d'œil du porteur du projet suffit à clore ce point.
