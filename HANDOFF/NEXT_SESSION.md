...
**Action immédiate recommandée pour la prochaine session (avec Docker disponible) :** démarrer la stack complète, se connecter avec un compte de test, et vérifier visuellement `/chat` (avatars, indicateur de streaming, badges de sources, auto-scroll) avant de considérer la passe UX terminée.
