...
export async function POST(request: NextRequest) {
  ...
  const reponse = await fetch(`${ARHIA_API_URL}/api/workflows`, {
    method: "POST",
  ...
  if (!reponse.ok) {
    return NextResponse.json({ erreur: (donnees && donnees.title) || donnees || "Échec de l'instanciation." }, { status: reponse.status });
  }
