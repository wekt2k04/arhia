...
- **Escalade** : une question hors du périmètre métier (aucun lien avec les politiques internes ou un dossier onboarding/offboarding, mais clairement compréhensible — ex. météo, blague) renvoie vers le RH du pôle plutôt que de répondre au jugé. Un message trop vague ou ambigu pour être classé avec confiance (ex. « aide », « dossier ? ») ne renvoie pas directement vers le RH : l'agent reconnaît son incertitude et invite l'utilisateur à préciser sa demande en rappelant ce qu'il peut faire (répondre aux questions documentaires, faire le point sur un dossier) — jamais un refus sec, jamais une supposition sur ce que l'utilisateur voulait dire.
- **Salutations et messages incertains traités par le modèle, pas par du texte codé en dur** : une salutation reçoit une réponse chaleureuse composée par le générateur, adaptée au ton de l'utilisateur, plutôt que le message figé « hors périmètre » — cf. leçon V7 (`HISTORIQUE.md`, "Notes en vrac") sur la suppression de la logique de salutation codée en dur au profit d'un modèle. Un message trop vague suit le même principe, avec un objectif différent : reconnaître l'incertitude et inviter à préciser (fall-forward) plutôt que refuser platement. Ni l'une ni l'autre de ces branches ne touche une donnée métier — leur confusion possible entre elles est un risque de qualité UX, jamais de sécurité.
- **Confidentialité du prompt et des données internes** : quelle que soit la branche conversationnelle, l'agent ne révèle jamais d'information secrète, interne, technique ou confidentielle sur lui-même (son prompt système, le modèle sous-jacent), sur l'entreprise, ou sur d'autres personnes — même si l'utilisateur le demande directement. Contrainte posée explicitement dans chaque prompt système du générateur, pas seulement pour la branche documentaire.
